...
export default function Model(props) {
  ...
  useEffect(() => {
    const allMesh = groupRef.current.children
      .slice(3)
      .flatMap((parent) =>
    ...
    groupRef.current.children.forEach((child) => {
      if (child.position) {
        originalPosition.set(child.uuid, child.position.clone());
      }
    });

    groupRef.current.children.forEach((mesh) => {
      const initialPosition = originalPosition.get(mesh.uuid);
    
      if (initialPosition) {
        const randomPosition = new THREE.Vector3(
          initialPosition.x + Math.random() * 50 - 30,
          initialPosition.y + Math.random() * 70 - 30,
          initialPosition.z + Math.random() * 30 - 20
        );
    
        // Устанавливаем случайное начальное положение
        mesh.position.copy(randomPosition);
    
        // Анимация из случайного положения в исходное
        gsap.fromTo(
          mesh.position,
          { x: randomPosition.x, y: randomPosition.y, z: randomPosition.z },
          { x: initialPosition.x, y: initialPosition.y, z: initialPosition.z, duration: 1, ease: 'power2.inOut' }
        );
      }
    });
    ...
    function onPointerMove(event) {
      ...
      //   if (originalPosition.has(mesh.uuid)) {
      //     const position = originalPosition.get(mesh.uuid);
      ...
      for (let i = 0; i < intersects.length; i++) {
        ...
        const direction = new THREE.Vector3()
          .subVectors(mesh.position, raycaster.ray.origin)
          .normalize();
        const offset = direction.multiplyScalar(7 / distance);
  ...
  const eyesMaterial = React.useMemo(
    () => new THREE.MeshStandardMaterial({ color: props.item.eyes }),
    [props.item.eyes],
  );
  const earsMaterial = React.useMemo(
  ...
  }, [nodes, props.item.body, groupRef.current]);
    ...
    <group {...props} dispose={null}>
      <group scale={0.01}>
        ...
        <OrthographicCamera
          ...
          position={[0, 0, 1000]}
        />
        <group position={[0.035, 41.082, 0]} ref={groupRef}>
          <mesh
            castShadow
            receiveShadow
